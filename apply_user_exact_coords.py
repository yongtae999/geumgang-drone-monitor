...
def update_user_exact_doowoong_coords():
    # User's exact coordinates: Latitude 36.836407, Longitude 126.196043
    lat_center = 36.836407
    lng_center = 126.196043
    ...
    for p in projects:
        if p["id"] == "doowoong":
            ...
            p["bearing"] = 35
            p["flight_tour"] = [
                { "lng": 126.1950, "lat": 36.8352, "alt": 140, "pitch": 60, "bearing": 35, "speed": 8.0, "name": "두웅습지 안내소 및 진입 상공" },
                { "lng": 126.1960, "lat": 36.8364, "alt": 95,  "pitch": 68, "bearing": 45, "speed": 6.5, "name": "미국수련 제거구역 (개방수면부 중심)" },
                { "lng": 126.1968, "lat": 36.8370, "alt": 90,  "pitch": 70, "bearing": 55, "speed": 6.0, "name": "황소개구리 포획구역 (수변 통발 15개소)" },
                { "lng": 126.1973, "lat": 36.8376, "alt": 120, "pitch": 65, "bearing": 60, "speed": 7.5, "name": "기타 정비구역 (탐방데크 및 완충지대)" },
                { "lng": 126.1960, "lat": 36.8364, "alt": 190, "pitch": 55, "bearing": 20, "speed": 10.0, "name": "두웅습지 람사르보호지역 6.7만㎡ 전체 부감" }
            ]

    with open(os.path.join(DATA_DIR, "projects.json"), "w", encoding="utf-8") as f:
        json.dump(projects, f, ensure_ascii=False, indent=2)

    # 2. Update GeoJSON Zones around exact coords (36.836407, 126.196043)
    zones = {
    ...
    # 3. Update Photos GPS around 36.836407, 126.196043
    photos = [
        {
    ...
    print(f"Applied User Exact Center: {lat_center} N, {lng_center} E")

if __name__ == "__main__":
    update_user_exact_doowoong_coords()
